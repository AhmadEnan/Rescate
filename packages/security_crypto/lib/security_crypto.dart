...
class MeshCrypto {
  ...
  Future<List<int>> encryptDirectMessage(String text, SimpleKeyPair senderIdentity, List<int> recipientPubBytes) async {
    return Profiler.span('crypto.encrypt', () async {
      Profiler.count('crypto.plaintext_bytes', text.length);
  ...
  Future<String> decryptDirectMessage(List<int> payload, SimpleKeyPair recipientIdentity) async {
    return Profiler.span('crypto.decrypt', () async {
      Profiler.count('crypto.ciphertext_bytes', payload.length);
