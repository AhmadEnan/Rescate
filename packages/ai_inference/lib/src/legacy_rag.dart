...
class LegacyRag {
  ...
  static const Map<String, List<String>> enExpansions = {
  ...
  /// Profiling: number of document×term scoring operations performed by the
  /// most recent [search] call (0 on cache hit). Read by LlmService for the
  /// per-turn `rag.search` trace step.
  static int lastSearchOps = 0;

  static Future<void> initialize() async {
    if (_chunks.isNotEmpty) return;
    try {
      await Profiler.span('rag.initialize', () async {
        ...
        Profiler.count('rag.chunks.loaded', _chunks.length);
  ...
  static bool _isProceduralQuestion(String question) {
    ...
    return q.contains('how') ||
        ...
        q.contains('steps') ||
        q.contains('treat') ||
        q.contains('ماذا أفعل') ||
        q.contains('كيف') ||
        q.contains('خطوات');
  }

  /// Scores the most relevant local window rather than counting repeated terms
  /// across the entire chunk. This keeps bibliographies and source lists from
  /// outranking the actual first-aid instructions later in a document chunk.
  static double _scoreChunk(
    String text,
    List<String> terms,
    List<String> rawWords, {
    required bool procedural,
  }) {
    final normalized = text.replaceAll(RegExp(r'\s+'), ' ').toLowerCase();
    final candidates = <int>[];
    for (final term in <String>{...terms, ..._actionTerms}) {
      var start = 0;
      while (true) {
        final index = normalized.indexOf(term, start);
        if (index < 0) break;
    ...
    final raw = rawWords.toSet();
    var best = 0.0;
    for (final candidate in candidates) {
      final windowStart = (candidate - 90).clamp(0, normalized.length).toInt();
      final end = (windowStart + windowSize)
          .clamp(0, normalized.length)
          .toInt();
      final window = normalized.substring(windowStart, end);
      var score = 0.0;
      var hasQueryTerm = false;
      for (final term in terms) {
        final occurrences = _countOccurrences(window, term);
        if (occurrences == 0) continue;
        hasQueryTerm = true;
        final weight = raw.contains(term) ? 4.0 : 2.0;
        score += weight * occurrences.clamp(1, 3);
      }
      if (!hasQueryTerm) continue;
      if (procedural) {
        for (final action in _actionTerms) {
          if (window.contains(action)) score += 6.0;
        }
      }
      if (window.contains('available at:') || window.contains('http')) {
        score -= 20.0;
      }
      if (window.contains('references:') ||
          (window.contains('sources:') && !procedural)) {
        score -= 10.0;
      }
  ...
  static List<Map<String, dynamic>> search(String question, {int topK = 5}) {
    ...
    if (_searchCache.containsKey(cacheKey)) {
      ...
      Profiler.count('rag.cache_hits', 1);
      lastSearchOps = 0;
      return cached;
    }

    final termsToSearch = isAr
        ? _getEnglishTermsFromArabic(question)
        : _getEnglishTerms(question);
    ...
    final procedural = _isProceduralQuestion(question);

    final scored = <Map<String, dynamic>>[];
    var scanOps = 0;
    for (final chunk in _chunks) {
      final text = chunk['text'] as String;
      scanOps += allSearchTerms.length;
    ...
    Profiler.count('rag.search.ops', scanOps);
    lastSearchOps = scanOps;
  ...
  static String buildPrompt({
    ...
    if (chunks.isEmpty) {
      ...
      for (var i = 0; i < chunks.length; i++) {
        final c = chunks[i];
        final text = _compactSnippet(
          (c['text'] as String).replaceAll(RegExp(r'\s+'), ' ').trim(),
          question,
        );
        compact.add('[${i + 1}]\n$text');
      }
    ...
    //
    // The <|think|> token selects Gemma 4's reasoning-trained path. Fully
    // generated reasoning produced better answers on the E2B checkpoint but
    // took several minutes on MT6893. In fast mode we therefore prefill one
    // short, fact-free thought and close the thought channel before native
    // generation starts. The model still produces the answer itself, while
    // the runtime decodes zero hidden thought tokens. Callers that explicitly
  ...
  static String _compactSnippet(String text, String question) {
    const maxLength = 180;
    if (text.length <= maxLength) return text;

    final queryTerms = _getEnglishTerms(question);
    final terms = <String>{...queryTerms, ..._actionTerms};
    ...
    for (final term in terms) {
      var searchStart = 0;
      while (true) {
        final index = normalized.indexOf(term.toLowerCase(), searchStart);
        if (index < 0) break;
        final start = (index - 35).clamp(0, text.length - maxLength).toInt();
        final window = normalized.substring(start, start + maxLength);
        var score = 0.0;
        for (final queryTerm in queryTerms) {
          score += 3.0 * _countOccurrences(window, queryTerm).clamp(0, 3);
        }
        if (_isProceduralQuestion(question)) {
          for (final action in _actionTerms) {
            if (window.contains(action)) score += 5.0;
          }
        }
        if (score > bestScore) {
          bestScore = score;
